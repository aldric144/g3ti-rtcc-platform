...
export default function MarinaLiveNetworkPage() {
  const [cameras, setCameras] = useState<MarinaCamera[]>([]);
  const [selectedCamera, setSelectedCamera] = useState<MarinaCamera | null>(null);
  ...
  const fetchCameras = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`${backendUrl}/api/cameras/marina/list`);
      if (!response.ok) throw new Error('Failed to fetch marina cameras');
  ...
  const loadSnapshot = useCallback(async (cameraId: string) => {
    try {
      ...
      const response = await fetch(`${backendUrl}/api/cameras/marina/${cameraId}/snapshot?t=${timestamp}`);
      if (response.ok) {
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-gray-400">Loading Marina Cameras...</span>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-3">
              ...
              Marina & Singer Island Live Feeds
            </h1>
            <p className="text-gray-400 mt-1">
              Live video feeds from Riviera Beach Marina, Singer Island, and Port of Palm Beach
            </p>
          </div>
            ...
            <div className="aspect-video relative">
              ...
              <div className="absolute top-4 right-4 flex items-center gap-2">
                ...
                <span className="px-2 py-1 bg-blue-500/20 text-blue-400 rounded text-xs">
                  {selectedCamera.refresh_interval}s refresh
                </span>
              </div>
            </div>
          </div>
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-2">
                ...
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className="text-gray-400 text-xs">{camera.category}</span>
                </div>
              </div>
            </button>
          ))}
