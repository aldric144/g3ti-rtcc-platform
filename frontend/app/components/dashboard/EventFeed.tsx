...
import { Target, Car, Video, AlertTriangle, Clock, MapPin, ChevronRight } from 'lucide-react';
...
export function EventFeed() {
  ...
  const filteredEvents = events.filter((event) => (filter === 'all' ? true : !event.acknowledged));

  return (
    <div className="card">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Live Event Feed</h2>
        ...
        {filteredEvents.map((event) => {
          ...
          return (
            <div
              key={event.id}
              className={clsx(
                'cursor-pointer rounded-lg border-l-4 p-4 transition-colors hover:opacity-90',
